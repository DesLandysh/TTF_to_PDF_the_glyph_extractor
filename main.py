...
def tab_to_lists(table_dict: dict) -> list:
    ...
    :return list[order_number, char, hex, dec, name] - order can be changed
    """
...
def create_pdf(table, output_file: str, font_family_name: str, font_path: str) -> None:
    ...
    pdf.add_page()
    pdf.add_font(family=font_family_name, fname=font_path, uni=True)
    pdf.set_font(family=font_family_name, size=font_size)
    col_width = pdf.w / 6
...
def main() -> None:
    font_path = "better-vcr-6.1.ttf"
    font_family_name = "RogueLike_RPG_MultyLang"

    from_font = ttLib.TTFont(font_path)
    ...
    # choose most valuable (chosen by testing)
    ...
    for index, in_row_tab in enumerate(tab_list):
        ...
        create_pdf(
            table=in_col_table,
            output_file=f'{font_family_name}_v{index}.pdf',
